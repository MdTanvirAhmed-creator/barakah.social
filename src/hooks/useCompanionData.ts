  ...
  const loadCompanionData = async () => {
    try {
      ...
      const { data: interactions } = await (supabase as any)
        ...
        .in('companion_connection_id', connections?.map((c: any) => c.id) || []);

      const { data: mentorships } = await (supabase as any)
      ...
      const avgStrength = connections && connections.length > 0
        ? connections.reduce((sum: any, c: any) => sum + c.connection_strength, 0) / connections.length
        : 0;
      ...
      setStats({
        ...
        is_mentor: mentorships?.some((m: any) => m.mentor_id === user.id) || false,
        is_mentee: mentorships?.some((m: any) => m.student_id === user.id) || false,
      });
    } catch (error) {
